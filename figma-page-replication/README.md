...
| 阶段 | 说明 |
|------|------|
| **P**reparation | 从 Figma 链接提取设计上下文 |
| **A**nalyze | 逐模块分析，判定策略（整图导出 vs 原生渲染） |
| **I**dentify | 收集策略 A 模块所需的高清资源 |
| **E**xecute | 按分析表逐模块构建页面代码 |
| **V**erify | 模块对照、视觉对比、交互检查 |
| **B**oundary | 6 类常见陷阱自动规避 |
...
- 6 类常见陷阱（1x 模糊、图片拉伸、模块遗漏等）的自动识别和规避
