#!/usr/bin/env python3
"""Compare two rendered images for high-fidelity design-to-code checks.

Outputs changed_pixel_ratio, MAE, RMSE, max_diff for quantitative QA.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-diff", help="Optional: save visual diff heatmap PNG")
...
def main() -> int:
    ...
    if args.output_diff:
        diff_norm = np.clip(rgb_diff / 255.0, 0, 1)
        heatmap = np.zeros((height, width, 3), dtype=np.uint8)
        heatmap[:, :, 0] = (diff_norm.max(axis=2) * 255).astype(np.uint8)
        heatmap[:, :, 3 if heatmap.shape[2] > 3 else 2] = 255
        diff_img = Image.fromarray(heatmap)
        diff_img.save(args.output_diff)
        result["diff_heatmap"] = args.output_diff
